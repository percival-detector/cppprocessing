...
class calibration_parameters:
    ...
    def __init__(self, parameter_name, height, width, lower_bound, upper_bound):
    ...
    def write_to_hdf5(self):
        ...
        dset = file.create_dataset(self.dataset_name, data=generate_array(self.height, self.width, self.lower_bound, self.upper_bound), dtype='i')
...
def generate_calib_files(height, width, text_file_name):
    ...
    file.write('calib_params_dataset_name:\"/data/data\"\n')
    file.write('is_transposed_calib_params:yes\n')
    file.close()
    
    #'calib_params_dataset_name':    "/data/data"
    param1 = calibration_parameters ('Gc', 7, height, 0, 20)
    param2 = calibration_parameters ('Oc', 7, height, 0, 20)
    param3 = calibration_parameters ('Gf', 7, height, 0, 20)
    param4 = calibration_parameters ('Of', 7, height, 0, 20)
    param5 = calibration_parameters ('ADU_to_electrons_conversion', width, height, 0, 20)
    
    param6 = calibration_parameters ('Gain_lookup_table1', width, height, 0, 20)
    param7 = calibration_parameters ('Gain_lookup_table2', width, height, 0, 20)
    param8 = calibration_parameters ('Gain_lookup_table3', width, height, 0, 20)
    param9 = calibration_parameters ('Gain_lookup_table4', width, height, 0, 20)
