...
BOOST_FIXTURE_TEST_SUITE (percival_ADC_decode_pf_combined_tbb_pipeline_test,fixture_frame_pf)

//RANGE TESTS
	BOOST_AUTO_TEST_CASE (should_throw_exception_when_output_and_input_dimensions_mismatch){
		percival_frame<float> wrong_size_frame;
		percival_frame_mem<float> tmp(250,250);
		wrong_size_frame = tmp;
		BOOST_REQUIRE_THROW(percival_ADC_decode_pf_combined_tbb_pipeline(src_frame,wrong_size_frame, calib_params, TEST_FRAME_WIDTH), dataspace_exception);
	}

	BOOST_AUTO_TEST_CASE (should_throw_exception_when_calibration_arrays_and_input_dimensions_mismatch){
		calib_params.Gc.set_frame_size(10, 7);
		BOOST_REQUIRE_THROW(percival_ADC_decode_pf_combined_tbb_pipeline(src_frame,des_frame, calib_params, TEST_FRAME_WIDTH), dataspace_exception);

		calib_params.Gc.set_frame_size(TEST_FRAME_HEIGHT, 5);
		BOOST_REQUIRE_THROW(percival_ADC_decode_pf_combined_tbb_pipeline(src_frame,des_frame, calib_params, TEST_FRAME_WIDTH), dataspace_exception);
...
//COMPUTATION TESTS
	BOOST_AUTO_TEST_CASE (try_correct_values){
			...
			float ADU_2e_conv = 7.1;
//
			*(calib_params.Gain_lookup_table1.data + 210) = G1;
			*(calib_params.Gain_lookup_table2.data + 210) = G2;
			*(calib_params.Gain_lookup_table3.data + 210) = G3;
			*(calib_params.Gain_lookup_table4.data + 210) = G4;
			*(calib_params.Gc.data + 7) = Gc;
			*(calib_params.Gf.data + 7) = Gf;
			*(calib_params.Oc.data + 7) = Oc;
			*(calib_params.Of.data + 7) = Of;

			*(src_frame.data + 210) = 0b0111110111110101; //0b011111 01111101 01;

			float coarse_calibrated = (Oc - 0b11111) * Gc;
			float fine_calibrated = (0b1111101 - Of) * Gf;
			float correct_result = (coarse_calibrated - fine_calibrated)* G2;
			BOOST_REQUIRE_NO_THROW(percival_ADC_decode_pf_combined_tbb_pipeline(src_frame, output_frame, calib_params, TEST_FRAME_WIDTH));
			BOOST_REQUIRE_CLOSE(correct_result, *(output_frame.data + 210), 0.01);
		}
...
BOOST_FIXTURE_TEST_SUITE (percival_ADC_decode_pf_combined_tbb_pipeline1_test,fixture_frame_pf)
...
//RANGE TESTS
	BOOST_AUTO_TEST_CASE (should_throw_exception_when_output_and_input_dimensions_mismatch){
		...
		percival_frame_mem<float> tmp(250,250);
		wrong_size_frame = tmp;
		BOOST_REQUIRE_THROW(percival_ADC_decode_pf_combined_tbb_pipeline1(src_frame,wrong_size_frame, calib_params, TEST_FRAME_WIDTH), dataspace_exception);
	}

	BOOST_AUTO_TEST_CASE (should_throw_exception_when_calibration_arrays_and_input_dimensions_mismatch){
		calib_params.Gc.set_frame_size(10, 7);
		BOOST_REQUIRE_THROW(percival_ADC_decode_pf_combined_tbb_pipeline1(src_frame,des_frame, calib_params, TEST_FRAME_WIDTH), dataspace_exception);
...
//
			...
			*(src_frame.data + 210) = 0b0111110111110101; //0b011111 01111101 01;
			...
			float fine_calibrated = (0b1111101 - Of) * Gf;
			float correct_result = (coarse_calibrated - fine_calibrated)* G2;
			BOOST_REQUIRE_NO_THROW(percival_ADC_decode_pf_combined_tbb_pipeline1(src_frame, output_frame, calib_params, TEST_FRAME_WIDTH));
			BOOST_REQUIRE_CLOSE(correct_result, *(output_frame.data + 210), 0.01);
		}
