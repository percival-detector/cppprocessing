/*
 * test_unit_gain_multiplication_pf.cpp
 *
...
//void percival_unit_gain_multiplication_pf(const percival_frame_mem<float> &, percival_frame_mem<float> &, const percival_calib_params & calib_params);
	...
	BOOST_AUTO_TEST_CASE (should_throw_exception_when_calibration_data_is_of_wrong_dimension){
		...
		BOOST_REQUIRE_THROW(percival_unit_gain_multiplication_pf(src_frame, calibrated, des_frame, calib_params), dataspace_exception);
	}
	...
	BOOST_AUTO_TEST_CASE (should_throw_exception_when_output_and_input_dimensions_mismatch){
		...
		BOOST_REQUIRE_THROW(percival_unit_gain_multiplication_pf(src_frame, wrong_size_frame, des_frame, calib_params), dataspace_exception);
		BOOST_REQUIRE_THROW(percival_unit_gain_multiplication_pf(src_frame, calibrated, wrong_size_frame, calib_params), dataspace_exception);
		BOOST_REQUIRE_THROW(percival_unit_gain_multiplication_pf(wrong_size_frame_int, calibrated, des_frame, calib_params), dataspace_exception);
	}
	...
	BOOST_AUTO_TEST_CASE (compare_computation_results){
		...
		*calibrated.data = 4.5;
		BOOST_REQUIRE_NO_THROW(percival_unit_gain_multiplication_pf(src_frame, calibrated, des_frame, calib_params));
