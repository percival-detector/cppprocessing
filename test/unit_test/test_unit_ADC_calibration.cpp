...
//void percival_unit_ADC_calibration(const percival_frame_mem<unsigned short int> & Coarse,const  percival_frame_mem<unsigned short int> & Fine,const percival_frame_mem<unsigned short int> & Gain, percival_frame_mem<float>& output, const percival_calib_params &);
...
//RANGE TESTS
	BOOST_AUTO_TEST_CASE (should_throw_exception_when_calibration_data_is_of_wrong_dimension){
		...
		BOOST_REQUIRE_THROW(percival_unit_ADC_calibration(coarse_frame, fine_frame, des_frame, calib_params), dataspace_exception);
	}

	BOOST_AUTO_TEST_CASE (should_throw_exception_when_output_and_input_dimensions_mismatch){
		percival_frame<unsigned short int> wrong_size_frame;	/*does not need the pointer data to be meaningful*/
		percival_frame<float> wrong_size_frame_float;
		wrong_size_frame.set_frame_size(10,10);

		BOOST_REQUIRE_THROW(percival_unit_ADC_calibration(wrong_size_frame, fine_frame, des_frame, calib_params), dataspace_exception);
		BOOST_REQUIRE_THROW(percival_unit_ADC_calibration(coarse_frame, wrong_size_frame, des_frame, calib_params), dataspace_exception);
		BOOST_REQUIRE_THROW(percival_unit_ADC_calibration(coarse_frame, fine_frame, wrong_size_frame_float, calib_params), dataspace_exception);
	}
	...
	BOOST_AUTO_TEST_CASE ( should_use_correct_location_in_calibration_array ){
		...
		*(calib_params.Of.data) = 4.0;
		...
		BOOST_REQUIRE_NO_THROW(percival_unit_ADC_calibration(coarse_frame, fine_frame, des_frame, calib_params));
