...
class fixture_test_unit_ADC_decode_frame{
public:

	percival_frame_mem<unsigned short int> tmp1;
	percival_frame_mem<unsigned short int> tmp2;
	percival_frame_mem<unsigned short int> tmp3;
	percival_frame_mem<unsigned short int> tmp4;
	...
	fixture_test_unit_ADC_decode_frame():
...
//percival_unit_ADC_decode(const percival_frame_mem<unsigned short int> &, percival_frame_mem<unsigned short int> & Coarse, percival_frame_mem<unsigned short int> & Fine, percival_frame_mem<short int> & Gain);

BOOST_FIXTURE_TEST_SUITE (percival_unit_ADC_decode_test,fixture_test_unit_ADC_decode_frame)
	BOOST_AUTO_TEST_CASE (should_throw_exception_when_output_and_input_dimensions_mismatch){
		...
		BOOST_REQUIRE_THROW(percival_unit_ADC_decode(src_frame, wrong_size_frame, fine_frame, gain_frame), dataspace_exception);
		BOOST_REQUIRE_THROW(percival_unit_ADC_decode(src_frame, coarse_frame, wrong_size_frame, gain_frame), dataspace_exception);
		BOOST_REQUIRE_THROW(percival_unit_ADC_decode(src_frame, coarse_frame, fine_frame, wrong_size_frame), dataspace_exception);
	}
	...
	BOOST_AUTO_TEST_CASE (should_throw_if_any_two_of_the_output_buffers_are_equal){
		BOOST_REQUIRE_THROW(percival_unit_ADC_decode(src_frame, coarse_frame, coarse_frame, gain_frame), datatype_exception);
		BOOST_REQUIRE_THROW(percival_unit_ADC_decode(src_frame, coarse_frame, coarse_frame, coarse_frame), datatype_exception);
		BOOST_REQUIRE_THROW(percival_unit_ADC_decode(src_frame, fine_frame, coarse_frame, coarse_frame), datatype_exception);
		BOOST_REQUIRE_THROW(percival_unit_ADC_decode(src_frame, coarse_frame, fine_frame, coarse_frame), datatype_exception);
